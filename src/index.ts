...
export const graphql = { namespace: 'agentSwarm', typeDefs: 'type Query { agentSwarmHealth: String!, agentSwarmLauncher: String! } type Mutation { agentSwarmRun(message: String!): String! }', resolvers: { Query: { agentSwarmHealth: () => AgentSwarm.health().status, agentSwarmLauncher: (_: unknown, __: unknown, ctx: RequestContext) => JSON.stringify(createPackageStatusPanel(ctx)) }, Mutation: { agentSwarmRun: async (_:unknown,args:{message:string},ctx:RequestContext)=>JSON.stringify(await AgentSwarm.run([{agentId:'default',input:args.message}], undefined, 10, ctx)) } }, migrations: ['migrations/0001_init.sql'] };
export function createPackage(): PackageModule { return { name: '@connectingmatrix/agent-swarm', version: '0.5.0', health: () => AgentSwarm.health(), graphql, migrations: graphql.migrations, launcher: createPackageStatusPanel, runtime: { AgentSwarm, observability: PackageObservability }, routes: [{ method: 'GET', path: '/agent-swarm/health', handler: () => AgentSwarm.health() }, { method: 'GET', path: '/agent-swarm/launcher', handler: (request) => createPackageStatusPanel((request as { context?: RequestContext }).context ?? {}) }] }; }
