...
export const graphql = { namespace: 'agentSwarm', typeDefs: 'type Query { agentSwarmHealth: String!, agentSwarmLauncher: String! } type Mutation { agentSwarmRun(message: String!): String! }', resolvers: { Query: { agentSwarmHealth: () => AgentSwarm.health().status, agentSwarmLauncher: (_: unknown, __: unknown, ctx: RequestContext) => JSON.stringify(createStubLauncher(ctx)) }, Mutation: { agentSwarmRun: async (_:unknown,args:{message:string},ctx:RequestContext)=>JSON.stringify(await AgentSwarm.run([{agentId:'default',input:args.message}], undefined, 10, ctx)) } }, migrations: ['migrations/0001_init.sql'] };
