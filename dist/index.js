...
const swarmRuntimeStatuses = new Map();
function applySwarmRuntimeStatus(event) { const previous = swarmRuntimeStatuses.get(event.processId); const row = { processId: event.processId, runId: event.runId ?? previous?.runId, status: event.status ?? previous?.status ?? 'running', progress: event.progress ?? previous?.progress ?? 50, updatedAt: nowIso(), logs: [...(previous?.logs ?? []), ...(event.logs ?? [])].slice(-500) }; swarmRuntimeStatuses.set(row.processId, row); processMonitoring?.heartbeat?.(row.processId, { status: row.status === 'failed' ? 'error' : 'ok', message: `Swarm ${row.status}`, metadata: { runId: row.runId } }); return row; }
...
Object.assign(AgentSwarm, { applyRuntimeStatus: applySwarmRuntimeStatus, runtimeStatus(runId) { const rows = [...swarmRuntimeStatuses.values()]; return runId ? rows.filter((row) => row.runId === runId) : rows; } });
export const graphql = { namespace: 'agentSwarm', typeDefs: 'type Query { agentSwarmHealth: String!, agentSwarmLauncher: String!, agentSwarmRuntimeStatus(runId: ID): String! } type Mutation { agentSwarmRun(message: String!): String! }', resolvers: { Query: { agentSwarmHealth: () => AgentSwarm.health().status, agentSwarmLauncher: (_, __, ctx) => JSON.stringify(createStubLauncher(ctx)), agentSwarmRuntimeStatus: (_, args) => JSON.stringify(AgentSwarm.runtimeStatus(args.runId)) }, Mutation: { agentSwarmRun: async (_, args, ctx) => JSON.stringify(await AgentSwarm.run([{ agentId: 'default', input: args.message }], undefined, 10, ctx)) } }, migrations: ['migrations/0001_init.sql'] };
export function createPackage() { return { name: '@connectingmatrix/agent-swarm', version: '0.5.0', health: () => AgentSwarm.health(), graphql, migrations: graphql.migrations, launcher: createStubLauncher, runtime: { AgentSwarm, observability: PackageObservability }, routes: [{ method: 'GET', path: '/agent-swarm/health', handler: () => AgentSwarm.health() }, { method: 'GET', path: '/agent-swarm/launcher', handler: (request) => createStubLauncher(request.context ?? {}) }] }; }
